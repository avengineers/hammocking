...
class MOCK_SECTION:
   ...
   def _func_impl(self, f: Tuple[str, str, List[Tuple[str, str]]]) -> str:
      ...
      func_decl = f"{return_type} {name}(" + ("void" if len(params) == 0 else ", ".join(f"{t} {n}" for t, n in params)) + ")\n"
...
class AUTOMOCKER:
   ...
   def read(self, input: Union[Path, str]) -> None:
      if issubclass(type(input), Path):   # Read a path
         cursor = Index.create(excludeDecls=True).parse(
         ...
         cursor = Index.create(excludeDecls=True).parse(
            path='~.c', unsaved_files=[('~.c', input)], options=TranslationUnit.PARSE_SKIP_FUNCTION_BODIES | TranslationUnit.PARSE_INCOMPLETE
         ).cursor
      for child in cursor.get_children():
         if child.spelling in self.symbols:
            ...
            if child.kind == CursorKind.VAR_DECL:
               ...
               mockup.add_function(resulttype, name, [(arg.type.spelling, arg.spelling) for arg in child.get_arguments()])
            else:
